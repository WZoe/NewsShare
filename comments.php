...
<div class="container mb-5">
    ...
    $stmt->execute();
    $stmt->bind_result($comment_id, $story_id, $content, $likes);
    ...
    while ($stmt->fetch()) {
        ...
        <div class="card-body text-truncate col-9">
            <h3 class="card-title">%s commented in <a href="%s">%s</a></h3>
            <p class="card-text">%s</p>
        </div>
                <div class="col-2 btn-group-vertical align-self-center">
        <button type="button" class="btn btn-secondary">Edit</button>
        <button type="button" class="btn btn-danger">Delete</button>
</div>
    </div></div></div>',
            $likes, $_SESSION['username'], $likes, $title, $content
        );
    }
    $stmt->close();
    ?>

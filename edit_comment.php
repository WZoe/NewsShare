...
if (!isset($_GET['story_id']) || !isset($_GET['comment_id'])) {
    ...
    $stmt1 = $mysqli1->prepare("SELECT content, user_id FROM comments WHERE id=?");
    $stmt1->bind_param('i', $comment_id);
    $stmt1->execute();
    $stmt1->bind_result($ori_content, $owner_id);
    $stmt1->fetch();
    $stmt1->close();
